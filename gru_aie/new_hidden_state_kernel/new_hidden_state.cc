...
void new_hidden_state(  input_stream<float> * __restrict cand_hidden_state_in,
    ...
    alignas(32) aie::vector<float, 8> old_hidden_state[H_VECTOR_SIZE/VECTOR_LANES];
    alignas(32) aie::vector<float, 8> z_v8, hhat_v8;
    alignas(32) float hhat[H_VECTOR_SIZE], z[H_VECTOR_SIZE];
    alignas(32) aie::accum<accfloat, 8> new_hidden_state[H_VECTOR_SIZE/VECTOR_LANES];
    ...
    for (;;){
        ...
        if (first_iteration_flag) {
            ...
            for (int i = 0; i < H_VECTOR_SIZE/VECTOR_LANES; i++){
                ...
                z_v8 = aie::load_v<8>((float*)&z[i]);
                hhat_v8 = aie::load_v<8>((float*)&hhat[i]);

                new_hidden_state[i] = aie::mul(aie::sub(float(1) ,z_v8), hhat_v8);

                new_hidden_state[i] = aie::mac(new_hidden_state[i], z_v8, aie::load_v<8>((float*)&h_init[i+VECTOR_LANES]));
            ...
            for (int i = 0; i < H_VECTOR_SIZE/VECTOR_LANES; i++){
                ...
                new_hidden_state[i] = aie::mac(new_hidden_state[i], z_v8, old_hidden_state[i]);

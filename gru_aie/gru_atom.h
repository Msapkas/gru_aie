...
public:
    ...
    add::kernel hhat_tanh_reduce;
    adf::port<adf::input> bh;
    // ------------------------------
    adf::kernel new_hidden_state;
    // ------------------------------
    ...
    gru_atom() {
            ...
            // plio_out = adf::output_plio::create(adf::plio_128_bits, "data/test_out");
            ...
            adf::connect(x_input.out[0], Wr_x.in[0]);
            ...
            adf::connect(x_input.out[0], Wz_x.in[0]);
            ...
            adf::connect(x_input.out[0], Wh_x.in[0]);
            ...
            adf::source(r_sigm_reduce) = "act_reduce/tanh_reduce.cc";
            ...
            new_hidden_state = adf::kernel::create(new_hidden_state);
            adf::source(new_hidden_state) = "new_hidden_state.cc";
            adf::runtime<ratio>(new_hidden_state) = 1;

            adf::connect(hhat_tanh_reduce.out[0], new_hidden_state.in[0]); 
            adf::connect(z_sigm_reduce.out[0], new_hidden_state.in[1]);

            // Output of the model
            adf::connect(new_hidden_state.out[0], hidden_state_out.in[0]);

            // Pass new hidden state to relevant kernels (feedback)
            adf::connect(new_hidden_state.out[0], Uz_hr.in[0]);
            adf::connect(new_hidden_state.out[0], Uz_h.in[0]);
            adf::connect(CONECT_HERE.out[0], Ur_h.in[0]);

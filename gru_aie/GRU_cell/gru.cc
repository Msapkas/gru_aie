...
int main(int argc, char ** argv){
    ...
    for (int i = 0; i < NKERNELS; i++){ID_param[i] = i*DIST_COEFF*VECTOR_LANES;}
    for (int i = 0; i < DIST_COEFF*X_VECTOR_SIZE*VECTOR_LANES; i++){W_test_params[i] = 0.001*i;}
    for (int i = 0; i < DIST_COEFF*H_VECTOR_SIZE*VECTOR_LANES; i++){U_test_params[i] = 0.005*i;}

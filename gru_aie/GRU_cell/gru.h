...
class gru : public adf::graph {
    ...
    // Pl IO
    adf::input_plio  PL_VECTOR_INPUT;
    adf::input_plio  PL_THIDDEN;
    adf::output_plio PL_HIDDEN_OUTPUT;
    ...
    gru () {
        ...
        // PL I/O
        PL_VECTOR_INPUT = adf::input_plio::create(adf::plio_128_bits, "data/test_in_arithmetic.txt");
        PL_THIDDEN = adf::input_plio::create(adf::plio_128_bits, "data/test_h_in.txt");
        PL_HIDDEN_OUTPUT = adf::output_plio::create(adf::plio_128_bits,"data/r_outputs.txt");
        ...
        for (int i = 0; i < NKERNELS; i++){
            // ------------------------------
            // R gates connections
            adf::connect<> (PL_VECTOR_INPUT.out[0], r_gates[i].x_input);
            adf::connect<> (PL_THIDDEN.out[0], r_gates[i].hidden_input);
        ...
        //     // ------------------------------
        //     // Z gate connections
        //     adf::connect<> (PL_VECTOR_INPUT.out[0], z_gates[i].x_input);
        ...
        //     adf::connect<> (PL_VECTOR_INPUT.out[0], candidate_hidden_gates[i].x_input);
        ...
        adf::connect<adf::stream> (r_aggregator_kernel.out[0], PL_HIDDEN_OUTPUT.in[0]);
        // adf::connect<adf::parameter>(r_aggregator_hidden_initialization, r_aggregator_mul_h_kernel.in[2]);
        ...
        //     adf::connect<adf::stream> (new_hidden_state_gate.out[0], candidate_hidden_gates[i].h_input);
        // }
        // // r aggregator needs hidden state to eprfomr r -mul- h
        // // adf::connect<adf::stream> (new_hidden_state_gate.out[0], r_aggregator_mul_h_kernel.in[1]);
        ...
        // adf::connect<adf::stream> (new_hidden_state_gate.out[0], PL_HIDDEN_OUTPUT.in[0]);
